...
*
*	@brief This file implments the RICS_Calculator class to calculate an image correlation function.
*
*	Calculation of an auto correlation function from a series of images. This file contains 
*	a single class that implements the calculation of the autocorrelation function in java,
*	interfaced for use in the ImageJ image manipulation suite. This autocorrelation function
*	can then be used for a RICS (Raster Image Correlation Spectrum) analysis of particle
*	number brightness and mobility. The RICS method was described by Michelle Digman et al. 
*	2005 in Measuring Fast Dynamics in Solutions and Cells with a Laser Scanning Microscope
...
/**
*	@brief A plugin class for ImageJ to calculate an image auto-correlation function.
*
*	This class is part of my implementation of RICS analysis for ImageJ. 
*	This class takes a stack of images as input and calculates the spatial auto-correlation 
*	function.
*	\f{
...
*	class.
*
*	@todo The class should be able to handle a single image and not just a stack of 
*			images, even if in practice for RICS analysis a stack is more appropriate.
*
*	@todo The initialization section should use a dialog to allow several different
*		  but related calculations intensity-correlation or fluctuation-correlation,
*		  cross-correlation or auto-correlation, with or without baseline subtraction 
*		  and allow user specification of the size of the correlation map.
*
*	@todo Sanity checks and error management should be implemented.
*
*	@todo Improve display of equations in Ref_Manual.
*
*	@todo Introduction in the User_Manual explaining equations, logic and acquisition 
...
public class RICS_calculator implements PlugInFilter {
  ...
  *	@return Flags determining the types of objects that can be treated STACK_REQUIRED + DOES_ALL
  *
  *	This function is used by ImageJ to determine what types of image that can be treated, for
  *	this plug in a stack of images is requred (STACK_REQUIRED), but the type of image (8bit, 16bit,
  *	grey scale or color) is not important (DOES_ALL). The function is also used to initialise
  *	the image with the appropriate structure.
  */
    this.image = image; 
  ...
  *	sizes are calculated, a section where the mean pixel intensity (the reference 
  *	for the correlation calculation) is computed and then the last section calculates 
  *	in real space the correlation map, finally the program creates an image of the 
  * auto-correlation map and displays it.
  *
  ...
  *					{n_{pixel}}
  *		\f}
  *	With the sum made over all pairs possible ensuring a constant number of pixels 
  *	for all different offsets. This is thus the image height and width reduced by 
  *	the size of the correlation map.
  */
