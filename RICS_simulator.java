...
public class RICS_simulator implements PlugInFilter {
  ...
  private ImagePlus image; 			/**< Place to strore the image initialized in 
  										setup(). 									*/ 
...
*	parameters ar calculated and space allocated, then in a loop over the images
    ...
    for (int f=0;f<Nt;f++) {				// Loop over images;
		...
		for( int p = 0; p < Npart; p++ ){	// Loop over particles...
			...
			for( int i = 0; i < Nx; i++ ){	// Add images of this particle to frame
	  			for( int j = 0; j < Ny; j++ ){
											// Calculate probability of excitation for 
											// the object
